...
const ZONE_PATTERNS: Record<string, ZonePattern> = {
  R4: {
    ...
    zoneIcon: '🔴',
    paragraphs: [
      '①現在、粘り強さ・情緒安定性・達成動機の3領域すべてにおいて、高ストレス環境への適合度が低い状態です。ただしこれは「能力がない」という意味ではなく、現時点での心理的コンディションと環境の組み合わせの問題です (Luthans et al., 2007)。',
      '②Dweck (2006) の成長マインドセット研究は、これらの特性が固定的なものではなく、適切な介入で変化可能であることを繰り返し示しています。Bandura (1997) の自己効力感理論も、小さな成功体験（mastery experience）が最も強力な変化の起点になることを実証しています。',
      '③現時点で最も注意すべきは、高ストレス・高プレッシャーの環境にいきなり配置されることです。心理的安全性 (Edmondson, 1999) が確保されていない環境では、持っている力を発揮する前に消耗してしまうリスクがあります。',
      '④推奨アクション：まず心理的安全性の高い環境（理解あるメンター、明確な期待値、定期的なフィードバック）を確保してください。その上で、週単位の小さな目標を設定し、達成→振り返り→次の目標のサイクルを回すことで、3〜6ヶ月後には数値が大きく変わる可能性があります。',
    ],
  },
  R1: {
    zoneId: 'R1',
    ...
    zoneIcon: '🔴',
    paragraphs: [
      '①出来事の原因を「自分の変えられない特性」に帰属させやすい傾向が強く出ています。Seligman (1986) の帰属スタイル研究では、このパターンが持続するとストレス下でのパフォーマンス低下と関連することが示されています。',
      '②重要なのは、帰属スタイルは「性格」ではなく「思考の習慣」だということです。Peterson & Seligman (1984) の研究で、悲観的帰属スタイルは認知行動的アプローチで変容可能であることが実証されています。',
      '③この帰属パターンが強い場合、失敗体験が「自分はダメだ」という信念を強化する悪循環に入りやすいリスクがあります。特に、フィードバックのない環境や孤立した状況ではこの傾向が強まります。',
      '④推奨アクション：日々の出来事について「原因は一時的か永続的か」「この領域だけの問題か全般的か」を意識的に書き出すジャーナリング (Seligman, 1991) が効果的です。また、メンターとの定期的な対話で帰属の偏りに気づく機会を作ることを推奨します。',
    ],
  },
  R2: {
    zoneId: 'R2',
    ...
    zoneIcon: '🔴',
    paragraphs: [
      '①粘り強さと情緒安定性の両方にまだ伸びしろがある状態です。ストレス環境では、困難に直面した際の感情の揺れと持続力の低下が同時に起きやすく、Luthans et al. (2007) のPsyCap理論ではこれを「心理的資本の複合的低下」と位置づけています。',
      '②Duckworth (2007) のGrit研究とCosta & McCrae (1992) の情緒安定性研究は、それぞれ独立した構成概念でありながら、低ストレス環境では互いを補完し合うことを示唆しています。',
      '③最大のリスクは、短期間に複数の困難が重なった場合の「二重負荷」です。粘り強さと感情制御の両方のリソースが同時に枯渇するため、回復に時間がかかる可能性があります。',
      '④推奨アクション：心理的安全性の高い環境 (Edmondson, 1999) で、まず感情面の安定を確保してください。その上でGritを育てるために、短期目標の設定→達成→承認のサイクルを意図的に設計することが効果的です。最初の90日間は週1回の1on1を推奨します。',
    ],
  },
  R3: {
    zoneId: 'R3',
    ...
    zoneIcon: '🔴',
    paragraphs: [
      '①2つの軸にまだ伸びしろがある状態です。これは「不合格」ではなく、現時点での環境適合度を示しています。Dweck (2006) の成長マインドセット研究が繰り返し示すように、これらの特性は可変的です。',
      '②Bandura (1997) の自己効力感理論では、低い自己効力感は「失敗体験の蓄積」と「モデリング機会の不足」が主因であることが示されています。環境要因を変えることで大きく改善可能です。',
      '③高ストレス環境への即座の配置はミスマッチのリスクが高いです。特に「放置型マネジメント」との相性が悪く、成果を出す前に離脱する可能性があります。',
      '④推奨アクション：段階的な負荷設計が鍵です。最初は確実に達成可能なタスクから始め、成功体験を積みながら徐々に難易度を上げてください。伴走者（メンター/バディ）の存在が成否を分けます。',
    ],
  },
  Y1: {
    zoneId: 'Y1',
    ...
    zoneIcon: '🟡',
    paragraphs: [
      '①帰属スタイルに揺らぎがある状態です。良い出来事に対しては楽観的に解釈できますが、悪い出来事に直面すると悲観的な解釈に傾きやすい傾向があります。Seligman (1991) はこれを「状況依存的帰属」と呼びます。',
      '②Peterson et al. (1982) のASQ研究では、このレンジの帰属スタイルは「可変域」にあり、環境と介入によって楽観方向にも悲観方向にもシフトする可能性があることが示されています。',
      '③リスクは「連続した失敗体験」です。悪い出来事が続くと帰属が悲観方向に固定化しやすいため、初期段階での成功体験設計が重要です。',
      '④推奨アクション：最初の90日間は「成功体験を意図的に作る」オンボーディング設計が効果的です。週1回の1on1で、出来事の解釈パターンをメンターと一緒に振り返ることで、楽観的帰属を強化できます (Seligman, 1991)。これを超えれば、Green人材と遜色ない活躍が期待できます。',
    ],
  },
  Y5: {
    zoneId: 'Y5',
    ...
    zoneIcon: '🟡',
    paragraphs: [
      '①複数の領域が境界域にある状態です。突出した弱点はないものの、Luthans et al. (2007) のPsyCap理論では、複数の心理的資本が同時にボーダーラインにある場合、環境ストレスの影響を受けやすいことが示されています。',
      '②一方で、特定の軸だけが低いケースと異なり、「全体的な底上げ」が可能な状態とも言えます。Bandura (1997) の自己効力感理論では、一つの領域での成功体験が他の領域にも波及する「般化効果」が確認されています。',
      '③最も避けるべきは、複数の高負荷タスクの同時アサインです。一つずつ着実にクリアすることで、全体的な自信の底上げにつなげてください。',
      '④推奨アクション：最初の3ヶ月間は週1回の1on1でフィードバックをもらい、小さな成功体験を意図的に積み上げてください。Duckworthのグリット研究やBanduraの自己効力感理論が示すように、これらの特性は環境と経験によって後天的に伸ばすことができます。良い上司やメンターとの出会いが鍵になります。',
    ],
  },
  Y2: {
    zoneId: 'Y2',
    ...
    zoneIcon: '🟡',
    paragraphs: [
      '①意欲と情緒安定性は十分な基盤がありますが、長期的な持続力にまだ伸びしろがあります。Duckworth & Quinn (2009) のGrit-S研究では、Consistency of Interest（興味の一貫性）とPerseverance of Effort（努力の粘り強さ）の2因子のうち、特に前者が課題である可能性があります。',
      '②Jachimowicz et al. (2018) は、Gritが最も効果を発揮するのは「情熱の対象が見つかっている」場合であることを示しました。粘り強さが低いのではなく、まだ本気で打ち込める対象に出会っていない可能性もあります。',
      '③注意点として、興味が移りやすいこと自体は探索期には健全です。ただし、短期間での環境移動が繰り返されると履歴書的にも心理的にも不利になりえます。',
      '④推奨アクション：最初の90日間はメンターとの週1回の1on1で進捗を可視化し、「ここまで来た」という実感を定期的に得られる仕組みを作ってください。小さな完了体験の積み重ねが、長期的な持続力の基盤になります (Duckworth, 2016)。',
    ],
  },
  Y3: {
    zoneId: 'Y3',
    ...
    zoneIcon: '🟡',
    paragraphs: [
      '①粘り強さと達成動機は十分ですが、ストレス下での感情の揺れに課題があります。Costa & McCrae (1992) のBig Five研究では、Neuroticism（情緒不安定性）が高い人は能力があっても、ストレス環境でパフォーマンスが一時的に低下しやすいことが示されています。',
      '②Gross (1998) の感情制御研究によれば、感情制御は認知的再評価（Cognitive Reappraisal）と表現抑制（Expressive Suppression）の2戦略があり、前者のトレーニングが長期的に効果的です。',
      '③リスクは「能力があるのにストレスで発揮できない」というパターンです。特にプレッシャーの強い場面（プレゼン、納期直前、上司からの叱責）で実力を出しきれない可能性があります。',
      '④推奨アクション：メンターのバッファ機能が重要です。直属の上司とは別に、感情面の相談ができるメンターを1名確保してください。また、マインドフルネスや認知的再評価の練習 (Gross & John, 2003) が、3ヶ月で有意な効果を示すことが報告されています。',
    ],
  },
  Y4: {
    zoneId: 'Y4',
    ...
    zoneIcon: '🟡',
    paragraphs: [
      '①粘り強さと情緒安定性の基盤は十分ですが、挑戦への意欲がまだ控えめです。McClelland (1961) の達成動機理論では、達成動機は「成功体験の蓄積」と「適度な難易度の課題設定」によって後天的に強化されることが示されています。',
      '②Lang & Fries (2006) のAMS-R研究では、Fear of Failure（失敗回避動機）がHope of Success（成功接近動機）を上回ると、挑戦を避ける行動パターンが強化されることが確認されています。',
      '③注意点として、達成動機が低いこと自体が問題ではなく、「挑戦を避けることで成長機会を逃す」悪循環が問題です。安定基盤があるからこそ、少し背伸びした挑戦が可能な状態にあります。',
      '④推奨アクション：段階的な目標設定（Locke & Latham, 2002）が最も効果的です。現在の実力の110%程度の課題を意図的にアサインし、達成時の承認を明確にすることで、「挑戦→成功→快感」のサイクルを構築してください。',
    ],
  },
  G2: {
    zoneId: 'G2',
    ...
    zoneIcon: '🟢',
    paragraphs: [
      '①非常に楽観的な帰属スタイルを持っており、困難な出来事に直面しても素早く立ち直る力があります。Seligman & Schulman (1986) の保険営業研究では、このレベルの楽観的帰属スタイルを持つ人は悲観群より37%高い業績を上げました。',
      '②この強みは特に、営業・起業・新規事業開発など、拒絶や失敗が日常的に発生する環境で大きなアドバンテージになります。Hmieleski & Baron (2009) の起業家研究でも、楽観的帰属は事業継続率と正の相関を持つことが確認されています。',
      '③ただしWeinstein (1980) が指摘する「非現実的楽観主義」のリスクには注意が必要です。リスクの過小評価や、フィードバックの軽視につながる場合があります。特に意思決定場面では、意図的に「悪いシナリオ」を検討する習慣が重要です。',
      '④推奨アクション：この楽観性を最大限に活かしつつ、事前検死法（Pre-mortem: Klein, 2007）を取り入れてください。プロジェクト開始前に「失敗するとしたら何が原因か」を洗い出すことで、楽観性とリスク管理のバランスが取れます。',
    ],
  },
  G3: {
    zoneId: 'G3',
    ...
    paragraphs: [
      '①非常に高い達成動機を持っています。McClelland (1961) の研究では、高い達成動機は起業家的行動、営業成績、リーダーシップ発揮と強い正の相関を持つことが繰り返し確認されています。',
      '②この強みは挑戦的な環境で最大限に発揮されます。Atkinson (1957) の期待-価値理論では、達成動機が高い人は「成功確率50%」程度の適度に困難な課題で最もモチベーションが上がることが示されています。',
      '③注意点として、達成動機が極めて高い場合、他者との協業よりも個人での成果を優先しがちです。McClelland (1975) は、達成動機が高すぎるリーダーは部下の育成をおろそかにしやすいことも指摘しています。',
      '④推奨アクション：個人目標とチーム目標を意識的にリンクさせてください。高い達成動機をチーム全体の成果向上に転換できれば、非常に高い組織貢献が期待できます。',
    ],
  },
  G4: {
    zoneId: 'G4',
    ...
    zoneIcon: '🟢',
    paragraphs: [
      '①粘り強さまたは情緒安定性が突出して高く、他の軸もバランスが取れています。Barrick & Mount (1991) のメタ分析では、情緒安定性（Emotional Stability）はほぼ全ての職種でパフォーマンスと正の相関を持つ数少ない特性として確認されています。',
      '②高いストレス耐性は、危機的状況やプレッシャーの強い環境で大きなアドバンテージになります。Luthans et al. (2007) のPsyCap研究では、Resiliency（回復力）が高い人はチーム全体の心理的安定に寄与する「アンカー」の役割を果たすことが示されています。',
      '③ただし、ストレス耐性の高さは時に「他者のストレスへの共感不足」として現れることがあります。自分が平気なことでも、周囲はそうでないかもしれません。Goleman (1995) の情動知能理論が示すように、共感力と自己制御は別の能力です。',
      '④推奨アクション：あなたの安定性はチームにとって貴重な資産です。意識的に「周囲が今どう感じているか」を確認する習慣をつけることで、高いストレス耐性と共感力を両立させた理想的なチームメンバーになれます。',
    ],
  },
  G1: {
    zoneId: 'G1',
    ...
    zoneIcon: '🟢',
    paragraphs: [
      '①高ストレス環境で活躍するための基盤が全ての軸でバランスよく備わっています。Seligman (1986) の帰属スタイル研究、Duckworth (2007) のGrit研究、Costa & McCrae (1992) のBig Five研究が示す「適応的な範囲」にあなたのスコアは収まっています。',
      '②バランス型の強みは「弱点がない」ことです。特定の状況でパフォーマンスが極端に落ちるリスクが低く、多様な環境に適応できます。Luthans et al. (2007) のPsyCap研究では、複数の心理的資本がバランスよく高い人は、単一の資本が突出している人よりも総合的なパフォーマンスが高いことが示されています。',
      '③一方で「何かに突出している」感覚が薄いため、自分の強みを自覚しにくい場合があります。ジョハリの窓 (Luft & Ingham, 1955) の概念が示すように、他者からのフィードバックで自分では気づかない強みを発見することが重要です。',
      '④推奨アクション：安定した基盤を活かしつつ、特に興味のある1つの軸を意図的に伸ばしてください。バランス型からスペシャリスト型への成長が、キャリアの差別化につながります。',
    ],
  },
}
